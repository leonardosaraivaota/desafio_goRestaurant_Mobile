...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      */
      // await api.get(`foods/${foodId}`).then(response => setFood(response.data));
      /*
      await api.get(`foods/${foodId}`).then(response => {
        // console.log(response.data);
        const foodFormatted = response.data.map(selectedFood => {
          return {
            ...selectedFood,
            formattedPrice: formatValue(selectedFood.price),
          };
        });
        console.log(foodFormatted);
        setFood(foodFormatted);
      });
      */
      /*
      const response = await api.get<Food[]>(`foods/${foodId}`);
      const foodFormatted = response.data.map(foodDetail =>
        formatValue(foodDetail.price),
      );
      console.log(foodFormatted);
      setFood(foodFormatted);
      */
      ...
      /*
      await api.get<Food[]>(`foods/${foodId}`).then(response => {
